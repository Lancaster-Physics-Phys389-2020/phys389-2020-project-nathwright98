...
List of scripts:
Particle.py - Defines a basic Particle object
ComplexParticle.py - Defines a ComplexParticle object that extends the Particle class, and is used for particles which undergo decay. It contains functions to handle the decay process, as well as important variables for decay.
DecayMode.py - Defines a DecayMode object, used to store data regarding the decay modes of particles.
DecaySimulation.py - Contains the bulk of the code used to iterate through steps of the simulation with the desired parameters, and iterate over the particles each step to perform the correct functions.
GUIController.py - Contains the code that creates the GUI, as well as functions relating to it.
RunGUI.py - The file that can be run in the terminal to open the GUI and begin using the program.